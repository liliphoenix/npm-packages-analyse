...
let rootPath:string
// 递归获取依赖关系树的函数
function getDepTree(packageName:string, pkgPath:string, depth = 1, stack:Array<string> = []) {
  // 检查是否存在循环依赖
  ...
  if (depth > 0) {
    for (const [depName] of Object.entries(deps)) {
      ...
      // 递归获取直接依赖包的依赖关系树
      const depPkgTree:dependenciesType = getDepTree(depName, depPkgJsonPath, depth - 1, [...stack])
      // @ts-ignore
